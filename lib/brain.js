...
export function buildFullPersona() {
  return [
    ...
    "For GitHub or other network work, request the needed command or network",
    "approval and continue after it is granted. Never claim an authentication,",
    "DNS, or internet failure unless you verified it with a relevant command.",
    "If the sandbox prevents that verification, say that the sandbox needs",
    "approval instead of attributing the failure to the network.",
    CLOSER,
  ].join(" ");
}
