...
export class LocalTarget extends ExecutionTarget {
  constructor({ enabled = false, safeRoots = [], shellEnabled = false, platform: targetPlatform = process.platform, commandRunner = runLocalCommand } = {}) {
    super({ id: "local", type: "local", platform: targetPlatform, enabled, capabilities: ["system-info", "filesystem-read", "filesystem-write", "process-list", "service-control", "shell"] });
  ...
  async execute(request) {
    switch (request.operation) {
      ...
      case "filesystem.write": return this.#write(request.arguments ?? {});
  ...
  async #write({ path, content, overwrite = false }) {
    const requested = text(path, "path");
    if (typeof content !== "string") throw new Error("content must be a string");
    if (Buffer.byteLength(content, "utf8") > MAX_WRITE_BYTES) throw new Error("content exceeds LocalTarget write limit");
    if (typeof overwrite !== "boolean") throw new Error("overwrite must be a boolean");

    // Resolve the existing parent rather than the file itself: a new file has no
    // real path yet. This also prevents writing through a symlinked directory
    // which points outside a configured safe root.
    const destination = resolve(requested);
    const parent = await realpath(dirname(destination));
    const roots = await Promise.all(this.safeRoots.map(async (root) => realpath(root).catch(() => null)));
    if (!roots.some((root) => root && inside(root, parent))) throw new Error("Path is outside LocalTarget safe roots");
    await writeFile(destination, content, { encoding: "utf8", flag: overwrite ? "w" : "wx" });
    return { path: destination, bytes: Buffer.byteLength(content, "utf8"), created: !overwrite };
  }
