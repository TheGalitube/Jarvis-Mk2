...
export class CodexAppSession {
  constructor({ cwd, bin = "codex", persona = "", onApproval, onEvent, env = process.env, autonomyMode = "supervised", fullAccess = false } = {}) {
    this.cwd = cwd;
    this.bin = bin;
    ...
    this.fullAccess = Boolean(fullAccess) || autonomyMode === "autonomous";
    this.child = null;
    this.buffer = "";
    this.nextId = 1;
  ...
  async #start() {
    ...
    const result = await this.#request("thread/start", {
      cwd: this.cwd,
      // Keep requests flowing through Jarvis: routine work is auto-accepted by
      // policy, while irreversible operations can still stop for confirmation.
      approvalPolicy: "untrusted",
      approvalsReviewer: "user",
      sandbox: this.fullAccess ? "danger-full-access" : "workspace-write",
  ...
  async #handleServerRequest(message) {
    const classification = classifyApproval(message, this.env, { autonomyMode: this.autonomyMode });
