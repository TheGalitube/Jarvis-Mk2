...
// These remain deliberate confirmation points even when the owner has enabled
// autonomous work. They are hard to undo or can affect the whole machine.
const ALWAYS_CONFIRM = [
  ...
  /\b(sudo|systemctl|shutdown|reboot|mkfs|dd)\b/i,
  /\b(terraform|pulumi)\s+(apply|destroy)\b/i,
  /\b(aws|gcloud|az)\s+(delete|destroy)\b/i,
];
const SENSITIVE_PERMISSION = /\b(delete|destroy|privileg\w*|root|sudo|secret|credential\w*|token\w*)\b/i;
...
export function classifyApproval(request, env = process.env, { autonomyMode = "supervised" } = {}) {
  const method = String(request?.method ?? "");
  const params = request?.params ?? {};
  const command = String(params.command ?? params.reason ?? "");
  const lower = command.toLowerCase();

  if (ALWAYS_CONFIRM.some((pattern) => pattern.test(command))) {
    return { level: "critical", reason: "destructive or privileged action" };
  }
  if (SENSITIVE_PERMISSION.test(String(params.reason ?? ""))) {
    return { level: "critical", reason: "sensitive permission request" };
  }
  if (autonomyMode === "autonomous") {
    return { level: "automatic", reason: "autonomous mode is enabled" };
  }
