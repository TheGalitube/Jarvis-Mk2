...
const STT_PROVIDERS = new Set(["auto", "chrome", "openai", "nemotron", "whispercpp"]);
export const OPENAI_STT_MODELS = new Set(["gpt-4o-transcribe", "whisper-1"]);
const LANGUAGE = /^(?:auto|[a-z]{2,3}(?:-[A-Z]{2})?)$/;
const AUTONOMY_MODES = new Set(["supervised", "autonomous"]);
...
export const DEFAULT_RUNTIME_CONFIG = Object.freeze({
  ...
  stt: {
    provider: "openai", fallbackToChrome: true,
    chrome: { enabled: true, language: "en-US" },
    openai: { model: "gpt-4o-transcribe", language: "de", timeoutMs: 20_000 },
...
export function validateRuntimeConfig(config) {
  ...
  if (!plainObject(config.agent) || !AUTONOMY_MODES.has(config.agent.autonomyMode)) {
    throw new Error("agent.autonomyMode must be supervised or autonomous");
  }
  ...
  if (!plainObject(stt) || !STT_PROVIDERS.has(stt.provider) || typeof stt.fallbackToChrome !== "boolean" || !plainObject(stt.chrome) || !plainObject(stt.openai) || !plainObject(stt.nemotron) || !plainObject(stt.whispercpp) || !plainObject(stt.gateway)) {
  ...
  if (!OPENAI_STT_MODELS.has(stt.openai.model) || typeof stt.openai.language !== "string" || !LANGUAGE.test(stt.openai.language) || !Number.isInteger(stt.openai.timeoutMs) || stt.openai.timeoutMs < 100 || stt.openai.timeoutMs > 60_000) {
    throw new Error("invalid OpenAI STT configuration");
  }
...
export function loadRuntimeConfig({ env = process.env, path, overrides } = {}) {
  ...
  const envOverrides = { execution: {}, voice: {}, stt: { chrome: {}, openai: {}, nemotron: {}, whispercpp: {}, gateway: {} } };
  ...
  if (env.JARVIS_OPENAI_STT_MODEL) envOverrides.stt.openai.model = env.JARVIS_OPENAI_STT_MODEL;
  if (env.JARVIS_OPENAI_STT_LANGUAGE) envOverrides.stt.openai.language = env.JARVIS_OPENAI_STT_LANGUAGE;
  if (env.JARVIS_OPENAI_STT_TIMEOUT_MS) envOverrides.stt.openai.timeoutMs = Number(env.JARVIS_OPENAI_STT_TIMEOUT_MS);
