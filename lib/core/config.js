...
const TARGET_CAPABILITIES = new Set(["system-info", "filesystem-read", "process-list", "service-control", "shell"]);
...
const STT_PROVIDERS = new Set(["auto", "chrome", "nemotron"]);
...
export function validateRuntimeConfig(config) {
  ...
  if (!plainObject(stt) || !STT_PROVIDERS.has(stt.provider) || typeof stt.fallbackToChrome !== "boolean" || !plainObject(stt.chrome) || !plainObject(stt.nemotron)) {
  ...
  if (stt.provider === "auto" && !stt.chrome.enabled && !stt.nemotron.endpoint) throw new Error("STT auto requires Chrome or Nemotron");
  return config;
}
...
export function publicRuntimeConfig(config) {
  return {
    ...
    stt: { provider: config.stt.provider, fallbackToChrome: config.stt.fallbackToChrome, chrome: { enabled: config.stt.chrome.enabled, language: config.stt.chrome.language }, nemotron: { configured: Boolean(config.stt.nemotron.endpoint), language: config.stt.nemotron.language } },
  };
}
...
export function loadRuntimeConfig({ env = process.env, path, overrides } = {}) {
  ...
  const envOverrides = { execution: {}, voice: {}, stt: { chrome: {}, nemotron: {} } };
