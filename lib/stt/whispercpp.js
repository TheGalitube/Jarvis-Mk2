...
export class WhisperCppProvider {
  constructor({ endpoint, language = "de", timeoutMs = 20_000, onEvent, fetchImpl = fetch } = {}) {
  ...
  async stop() {
    ...
    try {
      ...
      const response = await this.fetchImpl(whisperCppUrls(this.endpoint).inference, { method: "POST", body: form, signal: controller.signal });
      if (!response.ok) throw new Error("transcription-failed");
      const payload = await response.json();
      if (typeof payload?.text !== "string") throw new Error("transcription-failed");
      emit(this.onEvent, { type: "stt.final", provider: "whispercpp", text: payload.text.trim(), language: this.language });
      return true;
    } catch (error) {
