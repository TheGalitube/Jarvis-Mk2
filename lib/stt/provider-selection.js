...
export async function selectSttProvider(config, { nemotronHealth, whisperCppHealth } = {}) {
  ...
  if (stt.provider === "whispercpp" && healthyWhisperCpp) return { provider: "whispercpp", fallback: false };
  ...
  if (healthyWhisperCpp) return { provider: "whispercpp", fallback: false };
