...
// Telegram supplies an already encoded OGG/Opus file rather than the PCM
// frames used by the browser. Keeping this upload path here means both
// interfaces use the same OpenAI model, language, timeout and error contract.
export async function transcribeAudioFile({ apiKey, model = "gpt-4o-transcribe", language = "de", timeoutMs = 20_000, audio, filename = "voice.ogg", mimeType = "audio/ogg", fetchImpl = fetch } = {}) {
  if (!apiKey) throw new Error("OpenAI API key is not configured");
  if (!Buffer.isBuffer(audio) || audio.length === 0 || audio.length > MAX_FILE_BYTES) throw new Error("invalid audio file");
  ...
  try {
    ...
    const response = await fetchImpl(TRANSCRIPTIONS_URL, { method: "POST", headers: { Authorization: `Bearer ${apiKey}` }, body: form, signal: controller.signal });
    if (!response.ok) throw new Error("transcription-failed");
    const payload = await response.json();
    if (typeof payload?.text !== "string") throw new Error("transcription-failed");
    return payload.text.trim();
  } catch (error) {
    if (error?.name === "AbortError") throw new Error("transcription timed out");
